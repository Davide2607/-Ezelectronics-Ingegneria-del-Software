    ...
    initRoutes() {
        ...
        this.router.get(
            ...
            (req: any, res: any, next: any) => {
                ...
                if (!validRoles.includes(role)) {
                    return res.status(422).json({ error: "UNAUTHORIZED_USER" });
                }
                next();
            },
        ...
        this.router.get(
            ...
            (req: any, res: any, next: any) => {
                ...
                // Controllo se l'utente sta cercando i propri dati
                
                if (!isAdmin && loggedInUser.username !== requestedUsername) {
        ...
        this.router.delete(
            ...
            (req: any, res: any, next: any) => {
             ...
             if (!isAdmin && requestedUsername !== loggedInUser.username) {
                return res.status(401).json({ error: "UNAUTHORIZED_USER" });
            }   
                
                // Se l'utente autenticato è un Admin, controlla se sta cercando di eliminare un altro admin
            if (req.user.role === "Admin") {
                this.controller.getUserByUsername(req.user,req.params.username)
                    .then((requestedUser:any) => {

                        if (requestedUser.role === "Admin") {
                            return res.status(401).json({ error: "Admins cannot delete other admins" });
                        }
            ...
            }else if(requestedUsername === loggedInUser.username){
                next();
            }
        
                // Se l'utente autenticato non è un amministratore e sta cercando di eliminare un altro utente,
                
                // L'utente autenticato può eliminare i propri dati 
        ...
        this.router.patch(
            ...
            (req: any, res: any, next: any) => {
             ...
             if (req.params.username.trim() === '') {
                ...
                if (!dayjs(req.body.birthdate, 'YYYY-MM-DD').isValid() || dayjs().isBefore(req.body.birthdate, 'day')) {
                    return res.status(400).json({ error: 'Invalid or future birthdate' });}

                if (isAdmin && requestedUsername !== loggedInUser.username) {
                    return this.controller.getUserByUsername(loggedInUser,requestedUsername)
                        .then((requestedUser:any) => {
                            if (requestedUser.role === "Admin") {
                                return res.status(401).json({ error: "Admins cannot edit other admins" });
                            }
                            next();
                        })
                        .catch((err: any) => next(err));
